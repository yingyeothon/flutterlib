...
| --- | --- | --- |
| `bad_message` | any | not a JSON object with a string `type`, or a field of the wrong type (`dir` not a string) |
...
| `bad_scope` | `say`, `event` | `scope` is not one the channel allows |
...
| `too_long` | `say`, `event` | `text` over 1024 B, `name` over 64 B, payload over 8 KB |
...
flowchart TD
  ...
  kind -- "4002, 4005, 1001, 1011, 1006, other" --> recon["disconnected(willReconnect: true) → reconnecting"]
  kind -- "4000, 4004, 1000 lobby" --> stop["disconnected(false) → stopped(stop)"]
  kind -- "4003, 1003, 1009" --> bug["stopped(clientBug)"]
  kind -- "4001 on q" --> aborted["aborted"]
  kind -- "1000 on q" --> finished["finished"]
...
| `4900` | SDK-initiated (hello timeout, wrong subprotocol, oversized frame); never sent by the gateway |
